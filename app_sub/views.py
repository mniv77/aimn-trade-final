# =====================
# /app_sub/views.py (CLEAN)
# =====================
from flask import Blueprint, render_template, jsonify, request

from .models import LoopTarget
from sqlalchemy import delete
from .db import db_session
from shared_models import StrategyParam,  Trade
...
#@ui_bp.route("/")
#def home():
#    # keep it simple; your index.html already exists
...
# lazy pandas loader to avoid crashing on import at WSGI load time
def _pd():
    import importlib
...
@ui_bp.route("/scanner", endpoint="scanner")
def scanner():
    return render_template("aimn_flowing_scanner_auto.html")

@ui_bp.route("/trade-tester", endpoint="trade_tester")
def trade_tester():
    return render_template("trade_tester.html")

@ui_bp.route("/symbols", endpoint="symbols")
def symbols():
    return render_template("symbols.html")

@ui_bp.route("/symbol-api-manager", endpoint="symbol_api_manager")
...
def bar_count():
    ...
    ts = t.ts
    ts = ts.replace(tzinfo=timezone.utc) if getattr(ts, "tzinfo", None) is None else ts.astimezone(timezone.utc)
    count = int((df["t"] > ts).sum())
...
@api_bp.route("/alpaca_diag")
def alpaca_diag():
    import os
    from flask import jsonify
    try:
        # do not leak keys, just signal presence
        has_keys = bool(os.environ.get("ALPACA_KEY_ID")) and bool(os.environ.get("ALPACA_SECRET_KEY"))
        from .market_data import fetch_bars
        df = fetch_bars("BTC/USD", timeframe="1Min", limit=3)
        rows = int(len(df))
        first_ts = str(df["t"].iloc[0]) if rows else None
        return jsonify({"ok": True, "has_keys": has_keys, "rows": rows, "first_ts": first_ts})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)[:200]}), 500


@api_bp.route('/strategy_settings', methods=['GET'])
def strategy_settings():
    ...
    try:
        ...
        with conn.cursor() as c:
            c.execute('SELECT selected_symbols, params, updated_at FROM strategy_settings WHERE id=1')
            row = c.fetchone()
        conn.close()
        if not row:
            return jsonify({'ok': False, 'error': 'no_row'}), 404
        selected = json.loads(row['selected_symbols']) if isinstance(row['selected_symbols'], str) else row['selected_symbols']
        params = json.loads(row['params']) if isinstance(row['params'], str) else row['params']
        return jsonify({'ok': True, 'selected_symbols': selected, 'params': params, 'updated_at': str(row['updated_at'])})
    except Exception as e:
