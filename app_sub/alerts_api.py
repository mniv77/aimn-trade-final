...
def register_alerts_routes(app, db):
    ...
    def alerts_latest():
        try:
            # db is scoped_session
            result = db.execute(text("SELECT symbol, side, trail_pct, min_v_pct, price, created_at FROM alerts ORDER BY created_at DESC LIMIT 20"))
    ...
    def generate_alert(symbol):
        try:
            r = db.execute(text(f"SELECT trail_pct, min_v_pct FROM tuning_results WHERE symbol='{symbol}' AND mode='kiss_v3' ORDER BY created_at DESC LIMIT 1")).fetchone()
            if not r:
                return jsonify({"error": f"Tune {symbol} first"}), 404
            db.execute(text(f"INSERT INTO alerts (symbol, side, trail_pct, min_v_pct, price) VALUES ('{symbol}', 'BUY', {r[0]}, {r[1]}, 0)"))
            db.commit()
