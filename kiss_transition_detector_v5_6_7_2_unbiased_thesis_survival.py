"""KISS V5.6.7.2 — Coverage-aware Unbiased Thesis Survival Test.

RESEARCH ONLY.
No orders. No DB writes. No KISS engine changes. No RSI. No ML.

V5.6.7.1 still skipped every transition because it required both window
boundaries to exist within a five-minute alignment tolerance. The historical
5m dataset has gaps/session boundaries, so that requirement was too strict.

V5.6.7.2 fixes ONLY the coverage measurement:
  * use the earliest available completed 5m candle when the requested causal
    480-minute start is unavailable;
  * require a minimum causal history so the detector is not run with almost
    no context;
  * require the requested future evaluation horizon to be available;
  * record the actual history length and boundary offsets.

The candidate detector remains causal. Future candles are used ONLY to judge
whether exiting at that candidate was better than holding.
...
def nearest_completed_close(rows: Sequence[Dict[str, Any]], target: datetime) -> Tuple[Optional[int], Optional[float]]:
    """Nearest completed close at or before target, with reported offset."""
...
def candidate_signal(rows: Sequence[Dict[str, Any]], idx: int, prior: str) -> Tuple[bool, str]:
    """Causal candidate only. No future candles are consulted."""
...
def evaluate(rows: Sequence[Dict[str, Any]], idx: int, prior: str) -> Dict[str, Any]:
    """Future-only evaluation of a candidate."""
...
def run_case(rows: Sequence[Dict[str, Any]], event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Coverage-aware causal start: prefer T0-480m, otherwise use the earliest
    # available completed 5m candle, provided at least 120m of history exists.
    requested_start = t0 - timedelta(minutes=LOOKBACK_MINUTES)
    start, start_diff = nearest_completed_close(rows, requested_start)
    if start is None:
        return {"usable": False}

    # If the requested start is unavailable because of a session/data gap,
    # move forward through available history rather than discarding the case.
    ...
    # Future evaluation must really have 60m after each candidate. Candidates
    # near the end of the dataset can therefore be UNKNOWN; they are retained.
    candidates: List[Dict[str, Any]] = []
    for idx in range(start, end + 1):
        ok, reason = candidate_signal(rows, idx, prior)
        if not ok:
            continue
        candle_time = close_time(rows[idx])
        ev = evaluate(rows, idx, prior)
        candidates.append({
            "idx": idx,
            "time": candle_time,
            "minutes_to_t0": (t0 - candle_time).total_seconds() / 60.0,
            "boundary": idx == start,
            "reason": reason,
            "evaluation": ev,
...
def print_case(n: int, symbol: str, event: Dict[str, Any], result: Dict[str, Any]) -> None:
    ...
    print(f"  CAUSAL HISTORY: requested={result['requested_start']} actual={result['actual_start']} offset=-{result['start_offset']:.0f}m available={result['available_history']:.0f}m")
    print(f"  POST WINDOW: requested={event['time'] + timedelta(minutes=POST_MINUTES)} actual={result['actual_end']} offset=-{result['end_offset']:.0f}m")
    cs = result["candidates"]
    print(f"  CAUSAL CANDIDATES: {len(cs)}")
    for c in cs[:8]:
        b = " [HISTORY_BOUNDARY]" if c["boundary"] else ""
        ev = c["evaluation"]
...
def summarize(records: List[Dict[str, Any]], total: int, skipped: int) -> None:
    ...
    print(f"USABLE COMPLETE V5.6.7.2 WINDOWS: {len(records)}")
    ...
    for direction in ("LONG->SHORT", "SHORT->LONG"):
        ...
        for status in ("GOOD_EXIT", "FALSE_EXIT", "NEUTRAL", "UNKNOWN"):
            ...
            print(f"  {status:<12} {n}")
        if cs:
            print(f"  HISTORY-BOUNDARY CANDIDATES: {sum(c['boundary'] for c in cs)}")
    ...
    if valid:
        ...
        print(f"  AVG HOLD-60M GOOD EXIT: {avg_good:+.3f}%")
        print(f"  AVG HOLD-60M FALSE EXIT: {avg_false:+.3f}%")
    print("V5.6.7.2 KEY TEST: WAS EXITING HERE ACTUALLY BETTER THAN HOLDING?")
    print("V5.6.7.2 uses causal candidates and future-only truth evaluation.")
    print("V5.6.7.2 is research only — no trading rule or engine change.")
