"""KISS V5.6.7.1 — Unbiased Thesis Survival Test, alignment-corrected.

RESEARCH ONLY.
No orders. No DB writes. No KISS engine changes. No RSI. No ML.

V5.6.7 was correct conceptually but required exact 5m close timestamps at
both window boundaries. That caused all 128 known transitions to be skipped
when the stored 5m data did not land exactly on those requested boundaries.

V5.6.7.1 keeps the causal candidate/truth separation unchanged, but resolves
window boundaries to the nearest COMPLETED 5m candle within a small tolerance.
The actual candle used and its offset from the requested boundary are printed.

Candidate selection uses information available at the candidate candle only.
Future candles are used only for truth evaluation.
...
def completed_close_time(r: Dict[str, Any]) -> datetime:
...
def nearest_completed_close(rows: Sequence[Dict[str, Any]], target: datetime) -> Tuple[Optional[int], Optional[float]]:
    """Nearest already-completed 5m candle close within tolerance."""
    best: Optional[int] = None
    best_diff: Optional[float] = None
    for i, r in enumerate(rows):
        close_at = completed_close_time(r)
        if close_at > target:
            continue
        diff = (target - close_at).total_seconds() / 60.0
        if diff <= BOUNDARY_TOLERANCE_MINUTES and (best_diff is None or diff < best_diff):
            best = i
            best_diff = diff
...
def exact_close_index(rows: Sequence[Dict[str, Any]], target: datetime) -> Optional[int]:
    for i, r in enumerate(rows):
        if completed_close_time(r) == target:
            return i
    return None
...
def forward_closes(rows: Sequence[Dict[str, Any]], idx: int, minutes: int) -> List[int]:
    base_time = completed_close_time(rows[idx])
    out: List[int] = []
    for j in range(idx + 1, len(rows)):
        elapsed = (completed_close_time(rows[j]) - base_time).total_seconds() / 60.0
        if elapsed > minutes:
            break
        out.append(j)
    return out
...
def outcome_at(rows: Sequence[Dict[str, Any]], idx: int, prior: str, minutes: int) -> Optional[float]:
    j = exact_close_index(rows, completed_close_time(rows[idx]) + timedelta(minutes=minutes))
    if j is None:
        return None
...
def run_case(rows: Sequence[Dict[str, Any]], event: Dict[str, Any]) -> Dict[str, Any]:
    t0 = event["time"]
    prior = event["from"]
    start_target = t0 - timedelta(minutes=LOOKBACK_MINUTES)
    end_target = t0 + timedelta(minutes=POST_MINUTES)
    start, start_diff = nearest_completed_close(rows, start_target)
    end, end_diff = nearest_completed_close(rows, end_target)
    ...
    for idx in range(start, end + 1):
        ...
        if not ok:
            continue
        candle_time = completed_close_time(rows[idx])
    ...
    return {
        ...
        "start_actual": completed_close_time(rows[start]),
        "start_diff": start_diff,
        "end_target": end_target,
        "end_actual": completed_close_time(rows[end]),
        "end_diff": end_diff,
...
def print_case(n: int, symbol: str, event: Dict[str, Any], result: Dict[str, Any]) -> None:
    ...
    print(f"  WINDOW START target={result['start_target']} actual={result['start_actual']} offset=-{result['start_diff']:.0f}m")
    print(f"  WINDOW END   target={result['end_target']} actual={result['end_actual']} offset=-{result['end_diff']:.0f}m")
...
def summarize(records: List[Dict[str, Any]], total: int, skipped: int) -> None:
    ...
    print(f"USABLE COMPLETE V5.6.7.1 WINDOWS: {len(records)}")
    ...
    print("V5.6.7.1 KEY TEST: WAS EXITING HERE ACTUALLY BETTER THAN HOLDING?")
    print("V5.6.7.1 uses causal candidates and future-only truth evaluation.")
    print("V5.6.7.1 is research only — no trading rule or engine change.")
