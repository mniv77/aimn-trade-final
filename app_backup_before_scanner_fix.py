...
<body>
    ...
    <script>
        alert('JavaScript is running!');  // ADD THIS LINE AS THE FIRST LINE
        ...
        async function fetchScannerData() {
            console.log('fetchScannerData called!');
            const debugDiv = document.getElementById('debug-info');
            
            try {
                debugDiv.innerHTML = 'Fetching data...';
                
                // First add test data to verify table works
                if (scannerData.length === 0) {
                    ...
                    scannerData = [testData];
                    updateTable();
                    debugDiv.innerHTML = 'Test data loaded';
                }
                ...
                try {
                    ...
                    debugDiv.innerHTML = 'Got ' + data.new_entries.length + ' entries';
                    ...
                    if (data.new_entries && data.new_entries.length > 0) {
                        ...
                        // Update scanner data
                        scannerData = data.new_entries.slice(0, 3);
                        updateTable();
                        updateStats();
                    }
                } catch (parseError) {
                    debugDiv.innerHTML = 'Parse error: ' + parseError.message;
                }
            } catch (error) {
        ...
        setInterval(fetchScannerData, 3000);
    </script>
</body>
</html>
"""
